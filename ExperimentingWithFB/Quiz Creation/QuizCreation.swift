...
final class QuizCreationManager: ObservableObject {
    ...
    func readCreatedQuizzes(createdQuizzes: [Quiz]) async throws -> [Quiz]? {
        var quizName: String = ""
        var quiz: Quiz = Quiz()
        ...
        for quizCreated in quizzesCreated.documents {
            ...
            for n in 1...nbOfQuestions {
                ...
                guard let prompt = questionInfo["prompt"] as? String,
                      ...
                      let correctAnswer = questionInfo["correct_answer"] as? Set<String> else {
                    continue
                }
            ...
            let quizz = quiz
            quizzesUnwrapped.append(quizz)
            print("\(quizz)")
            
            print("\(quizzesUnwrapped)")
        }
    ...
    func editQuiz(chosenQuiz: Quiz, newName: String, newGrade: Int, newQuestions: [Question], newSubject: String, newNbOfQuestions: Int, newDifficultyLevel: String) async throws { // Still a work in progress
        ...
        if let quiz = try await Firestore.firestore().collection("users").document("\(user.uid)").collection("quizzes_created").document(chosenQuiz.name).getDocument().data() {
            guard var grade = quiz["grade"] as? Int,
                  var subject = quiz["subject"] as? String,
                  var difficultyLevel = quiz["difficulty_level"] as? String,
                  var nbOfQuestions = quiz["nb_of_questions"] as? Int else { // I don't really know about how this change will be monitored
                return
            }
            
            for n in 1...nbOfQuestions {
                if let question = quiz["question_\(n)"] as? [String: Any] {
                    guard var answers = question["answers"] as? [String: Bool],
                          var correctAnswers = question["correct_answer"] as? Set<String>,
                          var prompt = question["prompt"] as? String,
                          var questionAnswered = question["question_answered"] as? Bool else {
                        return
                    }
            ...
            if newNbOfQuestions != nbOfQuestions {
                nbOfQuestions = newNbOfQuestions
            }
